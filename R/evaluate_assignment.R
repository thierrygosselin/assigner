...
#' @description
#' Evaluate population-assignment performance using random or FST-ranked marker
#' subsets, with optional individual subsampling and cross-validation.
...
#'   \href{https://github.com/eriqande/gsi_sim}{gsi_sim}, developed by assigner
#'   co-author Eric C. Anderson for genetic stock identification and simulation,
#'   or
...
#' }
#'
#' @section Marker selection and high-grading bias:
#' Anderson (2010) demonstrated that assignment accuracy is biased upward when
#' the same individuals are used both to select an informative marker panel and
#' to evaluate that panel. Ordinary leave-one-out assignment does not remove
#' this high-grading bias because the focal individual has already influenced
#' marker ranking. With `markers.sampling = "ranked"`, use a genuine
#' Training-Holdout-Leave-one-out design. Holdout individuals must not influence
#' FST ranking, the number of markers selected, thresholds, parameter tuning, or
#' other analytical choices. Use `thl = "all"` only for development or
#' descriptive exploration, not to estimate future assignment accuracy.
#'
#' @param data Several input format are accepted. assigner uses \pkg{genometranslator}
#' \code{\link[genometranslator]{read_genome}} to import the data.
#' See function documentation for more details.

#' @inheritParams genometranslator::read_genome
...
#' This argument subsamples individuals to evaluate and reduce the influence of
#' unequal reference-population sizes. Unequal sizes can make allele-frequency
#' estimates more precise in large strata and bias assignment toward them.
...
#' This is best used as an exploratory sensitivity analysis: repeat subsampling
#' and compare assignment results with the full dataset. For low-coverage GL/PL
#' data, equal numbers do not necessarily provide equal information; use the
#' effective sample sizes returned by \code{\link{assign_individuals}} to guide
#' population or coverage equalization.
...
#' differentiation rather than technical or sampling structure. Manel et al.
#' (2005) emphasize matching the method and its assumptions to the biological
#' question. Before running or interpreting this evaluation, examine:
#' \itemize{
#'   \item \strong{Candidate sources and strata.} Candidate populations must be
#'     defined coherently and sampled representatively. An unsampled true source,
#'     pooled substructure, admixture, or an unsuitable choice of strata can
#'     produce a forced but misleading assignment.
#'   \item \strong{Linkage disequilibrium.} Highly linked markers provide
#'     correlated evidence and can cause a genomic region to be counted many
#'     times. Thin or prune markers, work with justified linkage blocks, and
#'     compare results across alternative panels. Any marker ranking or pruning
#'     used to evaluate performance must be learned from training data only.
#'   \item \strong{Hardy-Weinberg departure.} Strong departure can indicate
#'     genotyping error, paralogs, inbreeding, family structure, admixture, or
#'     incorrectly combined populations. Diagnose the cause rather than applying
#'     an automatic Hardy-Weinberg filter.
#'   \item \strong{Genotyping error.} Allelic dropout, low depth, duplicated or
#'     paralogous loci, inconsistent allele coding, and platform-specific errors
#'     can create false population signals. Use technical replicates and inspect
#'     depth, allele balance, reproducibility, and error patterns.
#'   \item \strong{Missingness and batch effects.} Missingness, sequencing depth,
#'     extraction batch, library, lane, and genotype-processing history should
#'     not be confounded with strata. A random holdout can preserve this
#'     confounding and report excellent but non-transferable accuracy. When
#'     possible, evaluate on a separate batch, collection, site, or time period.
#'   \item \strong{Duplicates and relatedness.} Duplicate samples or close
#'     relatives shared between training and holdout data leak information and
#'     inflate performance. Split related groups together when family structure
#'     is present.
#'   \item \strong{Rare variants and sample size.} Low minor-allele counts are
#'     unstable when LOO or THL removes individuals, and small or uneven source
#'     samples yield imprecise allele frequencies and asymmetric power. Examine
#'     sensitivity to marker and sample composition.
#'   \item \strong{Independent validation.} Report confusion among every source,
#'     not only overall success. Repeat the analysis across random seeds and
#'     marker panels, and prefer external validation when the intended samples
#'     differ from the reference data in space, time, laboratory batch, or
...
#' \href{https://thierrygosselin.github.io/radr/}{radr} or another suitable
...
#' The \code{adegenet} engine is supplied by the CRAN package of the same name.
#' The alternative \href{https://github.com/eriqande/gsi_sim}{gsi_sim} engine,
#' developed by assigner co-author Eric C. Anderson, is an external command-line
#' program and is not installed by \pkg{assigner}.
#' Follow the complete source-installation instructions in the package
#' get-started vignette and validate the executable with
#' \code{\link{check_gsi_sim}} before selecting
#' \code{assignment.analysis = "gsi_sim"}.

# Return ------------------------ ----------------------------------------------
#' @return Depending on arguments selected, several folders and files are written:
#' \enumerate{
#' \item \code{assigner_evaluate_assignment_args_date@time.tsv}: For reproducibility,
...
#' @references Anderson EC (2010). Assessing the power of informative subsets of
...
#' Ecology & Evolution, 20(3), 136-142. doi:10.1016/j.tree.2004.12.004.
...
#' @seealso [assign_individuals()],
#' \href{https://github.com/eriqande/gsi_sim}{gsi_sim}, and
#' \href{https://github.com/eriqande/rubias}{rubias}
#'
#' `gsi_sim`, developed by assigner co-author Eric C. Anderson, is used here to
#' evaluate individual assignment and marker-panel performance. `rubias`,
#' developed by Eric C. Anderson and Ben Moran, addresses the related but
#' distinct conditional GSI problem of jointly estimating mixed-stock
#' proportions and individual posterior assignments. See [assign_individuals()]
...
evaluate_assignment <- function(
  ...
  .start <- tgbase::startup(
    f.name = "evaluate_assignment", package = "assigner", verbose = verbose
  )
  ...
  assignment.dots <- tgbase::resolve_dots(
    dots = rlang::dots_list(..., .homonyms = "error", .check_assign = TRUE),
    allowed = c("adegenet.dapc.opt", "adegenet.n.rep", "adegenet.training",
      "folder", "filename", "keep.gsi.files", "random.seed",
      "whitelist.markers", "full.y.range"),
    defaults = list(
      adegenet.dapc.opt = "optim.a.score", adegenet.n.rep = 30L,
      adegenet.training = 0.9, folder = NULL,
      filename = "assignment_data.txt", keep.gsi.files = FALSE,
      random.seed = NULL, whitelist.markers = NULL, full.y.range = FALSE
    ),
  ...
  if (assignment.analysis == "gsi_sim") check_gsi_sim()
  ...
  directory <- tgbase::generate_folder(
    folder = rad.folder,
    path.folder = folder,
    prefix.int = FALSE,
    append.date = TRUE,
  ...
  tgbase::write_tgbase_tsv(
    data = assignment.dots,
    path.folder = directory,
    filename = stringi::stri_join(
      "assigner_evaluate_assignment_args_", file.date, ".tsv"),
    internal = FALSE,
  ...
  if (!rlang::has_name(input, "GT")) {
    input %<>% genometranslator::calibrate_alleles(data = ., gt = TRUE) %$% input
  }

  # Strata and pop levels ------------------------------------------------------
  input <- genometranslator::change_pop_names(data = input, pop.levels = pop.levels)
  strata <-  genometranslator::generate_strata(data = input, pop.id = TRUE)
...
assignment_subsamples <- function(
  ...
  if (assignment.analysis == "adegenet") {
    ...
    genind.object <- genometranslator::write_genind(data = input)
  }
  ...
  if (markers.sampling == "random") {
    ...
    assignment.res <- NULL
    assignment.res <- tgbase::parallel_map(
  ...
  if (markers.sampling == "ranked") {
    ...
    assignment.res.summary <- tgbase::parallel_map(
        .x = iterations.list,
...
assignment_ranking <- carrier::crate(function(
  ...
  i.folder <- tgbase::generate_folder(
    folder = stringi::stri_join("assignment_", i),
    path.folder = directory.subsample,
    prefix.int = FALSE,
    internal = FALSE,
    append.date = FALSE,
...
# assignment_gsi_sim------------------------------------------------------------
#' @title Assignment with Eric C. Anderson's gsi_sim
#' @description Run population assignment with
#'   \href{https://github.com/eriqande/gsi_sim}{gsi_sim}, developed by assigner
#'   co-author Eric C. Anderson. Eric also developed the original wrapper used
#'   by \pkg{assigner}.
...
assignment_gsi_sim <- function(
  ...
  input.gsi <- genometranslator::write_gsi_sim(
    data = input %>%
  ...
  status <- system2(
    gsi_sim_binary(),
    args = c("-b", shQuote(input.gsi), "--self-assign"),
    stdout = output.gsi,
    stderr = TRUE
  )
  if (!identical(status, 0L)) {
    rlang::abort(paste0("gsi_sim failed with exit status ", status, "."))
  }
