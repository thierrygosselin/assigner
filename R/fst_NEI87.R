...
#' \code{\link[genometranslator]{read_genome}}. See details for more info.
#'
#' \emph{How to get a tidy data frame ?}
#' \href{https://github.com/thierrygosselin/genometranslator}{genometranslator}
#' \code{\link[genometranslator]{read_genome}} can import supported genomic data formats
...
#' the function \pkg{genometranslator} \code{\link[genometranslator]{read_genome}} searches
...
#' (e.g. from a VCF; see \pkg{genometranslator} \code{\link[genometranslator]{read_genome}}). A minimum of 4 columns
...
#'
#' \emph{How to get a tidy data frame ?}
#' \pkg{genometranslator} \code{\link[genometranslator]{read_genome}} can import supported genomic data formats
...
fst_NEI87 <- function(
  ...
  .start <- tgbase::startup(
    f.name = "fst_NEI87", package = "assigner", verbose = verbose
  )
  ...
  input <- genometranslator::read_genome(data = data)
  ...
  input <- genometranslator::change_pop_names(data = input, pop.levels = pop.levels, pop.labels = pop.labels)
...
compute_fst_nei <- function(x, ci = FALSE, iteration.ci = 100, quantiles.ci = c(0.025,0.975), digits = 9) {
  ...
  x <- radr::filter_common_markers(data = x)

  # Removing monomorphic markers------------------------------------------------
  x <- radr::filter_monomorphic(data = x)
  ...
  x <- x %>%
    ...
    dplyr::select(-GT) %>%
    tgbase::trans_long(x = ., cols = c("MARKERS", "INDIVIDUALS", "STRATA"), names_to = "ALLELES", values_to = "GT")
...
fst_nei_subsample <- function(
  ...
  if (pairwise) {
    ...
    fst.all.pop <- tgbase::parallel_map(
      .x = list.pair,
      .f = pairwise_fst_nei,
      flat.future = "dfr",
      split.vec = FALSE,
    ...
    upper.mat.fst <- pairwise.fst %>%
      ...
      tgbase::trans_wide(x = ., formula = "POP1 ~ POP2", values_from = "NEI_FST_P", values_fill = "" ) %>%
    ...
    if (ci) {
      ...
      lower.mat.ci <- pairwise.fst %>%
        ...
        tgbase::trans_wide(x = ., formula = "POP1 ~ POP2", values_from = "CI", values_fill = "" ) %>%
        dplyr::rename(POP = POP1)
