...
To install R v3.6.1 **"Action of the Toes"** released on 2019-07-05 download the
installer and follow the instructions
```{bash eval=FALSE}
cd Downloads
wget https://cran.r-project.org/bin/macosx/R-3.6.1.pkg
```
...
If you want to use more loci during your simulations (default is 1001),
...
```{bash eval=FALSE}
sed -i -e 's/#define MAXLOCI               1001/#define MAXLOCI               20000/g' rmetasim-master/src/const.h
```
...
```{bash eval=FALSE}
cd ~/Downloads
curl -O https://cran.r-project.org/src/contrib/randomForestSRC_4.6-14.tar.gz
tar -zxvf randomForestSRC_4.6-14.tar.gz
cd randomForestSRC
autoconf
cd ..
...
cd openmpi-1.6.5
export TMPDIR=/tmp
./configure F77=gfortran --prefix=/usr/local -openmp
make -j 12
sudo make install
```
...
knitr::include_graphics("/Users/thierry/Library/Mobile Documents/com~apple~CloudDocs/r_packages/radiator/vignettes/terminal_folder_shortcut.png")
```
...
```{r automator.png, echo=FALSE}
knitr::include_graphics("/Users/thierry/Library/Mobile Documents/com~apple~CloudDocs/r_packages/radiator/vignettes/automator.png")
```
...
knitr::include_graphics("/Users/thierry/Library/Mobile Documents/com~apple~CloudDocs/r_packages/radiator/vignettes/copy_path.png")
```
