...
#In Terminal
cd Downloads
sudo tar -zxvf gcc-8.3-bin.tar.gz -C /usr/local --strip-components=2
```

```{bash eval=FALSE}
sudo rm gcc-8.3-bin.tar.gz
gcc -v
```
...
```{bash eval=FALSE}
#In browser
http://releases.llvm.org/8.0.0/clang+llvm-8.0.0-x86_64-apple-darwin.tar.xz
#In Terminal
cd Downloads
sudo tar -xzvf clang+llvm-8.0.0-x86_64-apple-darwin.tar.xz -C/usr/local --strip-components=1
```


```{bash eval=FALSE}
sudo rm clang+llvm-8.0.0-x86_64-apple-darwin.tar.xz
clang -v
```
...
### Download
Download the latest github release (3.1.10) of Allan Strand's **rmetasim**
```{bash eval=FALSE}
cd ~/Downloads
