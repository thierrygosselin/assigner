# Pairwise and overall Fst with confidence intervals + building phylogenetic tree

## Objectives

- learn how to run the function
...
- learn how to integrate the function with other package, e.g. to build
  a phylogenetic tree
...
polymorphic markers in common between the populations.

When your project involves \> 2 populations that evolved more by neutral
processes (e.g. genetic drift) than by natural selection, filtering to
keep only the markers in common between your populations will
drastically reduce the number of markers, and consequently your genome
coverage for the estimate. Strategically, you should reduce the
...
easily tested with `radiator::filter_common_markers` or in stacks
population module.

The overall fst and fis estimates will have reduce genome-wide coverage,
because the computation is conducted with shared markers accross all
your populations. However, for the pairwise comparisons, the markers
will be tallied automatically for the pair. This usually results in more
legitimate markers (i.e. less filtering bias) being used during
computation, providing better genome-wide estimates for that pair.
...
[`assigner::fst_WC84`](https://thierrygosselin.github.io/assigner/reference/fst_WC84.md)
first run: overall and pairwise, but without boostrap (i.e. no
confidence intervals). Get the function documentation with:
`??assigner::fst_WC84`.
...
1.  Get access to specific content with `$`:
    - `$sigma.loc`: the variance components per locus: `lsiga`: among
      populations, `lsigb`: among individuals within populations,
    ...
    - `$pairwise.fst`: the pairwise fst in long/tidy data frame and
      number of markers per pairwise comparison

    - `$pairwise.fst.upper.matrix`: the pairwise fst in a upper triangle
      matrix

    - `$pairwise.fst.full.matrix`: the pairwise fst matrix (duplicated
...
2.  To view the pairwise Fst values in a tibble with 4 columns:
...
1.  The first 10 parwise comparisons in `df`
    ...
    *N_MARKERS*: is the number of markers used between the pairs to
    calculate the Fst.
...
1.  So let’s make another run with `fst_WC84`, but this time will ask
    for the confidence intervals and the heatmap:
...
1.  To have this in a handy table for publication, we will create a new
    object `pairwise.fst.ci.df`:
...
1.  Or even better, use the heatmap of Fst and CI values…:
    ![Heatmap of pairwise FST estimates with confidence
    intervals](fst_WC84_heatmap_fst.png)
...
1.  To build the tree we will need to install
...
1.  Several vignettes are available to get to know
    [ggtree](http://guangchuangyu.github.io/ggtree/)

2.  Build a very basic tree figure:
...
![Example population tree constructed from pairwise FST
distances](assigner_fst_tree_example.png)
...
G Yu, D Smith, H Zhu, Y Guan, TTY Lam, ggtree: an R package for
visualization and annotation of phylogenetic tree with different types
of meta-data. revised.
