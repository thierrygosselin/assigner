...
against alternatives. To make differences and bottlenecks really stand
out we will use a simulated dataset and an empirical RADseq dataset both
with more than 10K markers.
...
- You might be interested in this package called
  [here](https://here.r-lib.org) or use `setwd("/YOUR/NEW/PATH/HERE/")`,
...
- [GenoDive macOS
  only](http://www.bentleydrummer.nl/software/software/GenoDive.md)
  (v.3.04)
...
We’re going to use the unfiltered vcf sunflower dataset from Owens et
al. 2016, kindly provided by Gregory Owens. This dataset as missing
data.

Owens GL, Baute GJ, Rieseberg LH. Revisiting a classic case of
...
2630-2643. <doi:10.1111/mec.13569>
...
- [strata](https://www.dropbox.com/s/406jleg3imwfaup/sunflower.strata.tsv?dl=0).
  What’s a strata file ? Read the
  [doc](https://thierrygosselin.github.io/radiator/reference/read_strata.html).

### Simulated datasets

After running this vignette once, I saw something strange in the
results. To explore this further, this second grind of testing add 2
additional datasets:
[data_assigner_sim_01](http://thierrygosselin.github.io/assigner/reference/data_assigner_sim_01.md)
and
[data_assigner_sim_02](http://thierrygosselin.github.io/assigner/reference/data_assigner_sim_02.md).
...
different Fst, admixture/membership probability.

## Generate the output files

We use
[radiator::genomic_converter](https://thierrygosselin.github.io/radiator/reference/genomic_converter.html)
to produce the required files or R objects:
...
- by default, `radiator::genomic_converter` will only keep polymorphic
  markers in common between the strata.
...
To learn more about the output folders and files:
[radiator::genomic_converter](https://thierrygosselin.github.io/radiator/reference/genomic_converter.html)

*Note on timing*

I could use fancy timing packages
([microbenchmark](https://github.com/joshuaulrich/microbenchmark),
[bench](http://bench.r-lib.org)), but for simplicity we’re just going to
use `system.time`.
...
Let’s start with the Gold Standard, Jérôme Goudet’s
[hierfstat](https://github.com/jgx65/hierfstat) package.
...
This is Eric Archer’s [strataG](https://github.com/EricArcher/strataG)
package.
...
- Kevin Keenan’s
  [diveRsity](http://diversityinlife.weebly.com/software.md) package
  requires the `genepop` file.
- the function is optimized with C++, so no need for a coffee (\< 1 min
  on my MBP)
...
- Xiuwen Zheng’s [SNPRelate](https://github.com/zhengxwen/SNPRelate)
  package is very fast.
- No out-of-the-box pairwise Fst analysis. So I implemented my own
  below.
...
- Inside Patrick Meirmans’s
  [GenoDive](http://www.bentleydrummer.nl/software/software/GenoDive.md)
  software, *import* or *drag* the `genepop` file
- Run: `Analysis -> Pairwise Differentiation -> F_st`, choose
  `1 permutations`.
- Save the results or copy/paste in MS Excel or text editor:
  **genodive.fst.tsv**.
- no need for a coffee, `GenoDive` is coded in Objective-C (~ \< 3 sec
  on my MBP)
...
- several manipulations of the results are required to get from GenoDive
  to R
...
I’m not particularly fond of VCFtools for Fst. Too much manipulation of
files. For the sake of the exercise I’m going to compare it quickly with
2 populations for the empirical dataset only.
...
PLINK users, note that the `--fst` option in [PLINK
v.1.9](https://www.cog-genomics.org/plink/1.9/basic_stats#fst) is
actually a port of VCFtools..

For me, comparison with VCFtools stops here, too painfull to conduct
...
understand how those VCFtools results compares with the other packages).
...
Learn more about
[`assigner::fst_WC84`](https://thierrygosselin.github.io/assigner/reference/fst_WC84.md)
with the dedicated
[vignette](http://thierrygosselin.github.io/assigner/articles/fst_confidence_intervals.md).
...
There’s obviously differences when we look at the pairwise comparisons
separately. The difference between the approaches taken in the
packages/software probably depends on numerous things:
...
- VCFtools: we used only 2 populations and timing is inside VCFtools
  only, it doesn’t account for manipulations of output.
- SNPRelate: No out-of-the-box pairwise Fst analysis, but very quick
  solution is proposed in this vignette.

## Conclusion

You could go and look at the 36 pairwise comparisons, but the end of the
story is very simple:

- For Fst, I’m definitely staying away from VCFtools and SNPRelate
  `MeanFst`.
- `SNPRelate` MeanFst as a systematic downward bias and the weighted
  estimate is usually a little higher.
- Empirical and simulated data: `assigner` and `hierfstat` have equal
  values down to a very large number of decimals!
- The packages show different colors with empirical and simulated data.
  - contrary to empirical data, with simulated data all the packages
    have the same results.
  - missing data is probably the stats driving those differences.
  - `assigner` and `hierfstat` show consistent behaviors in the presence
    of missing data.
  - high or low Fst in simulated data: doesn’t impact the packages.
  - `assigner` is different than the others: it transform `-` Fst to 0.
  - it should be worth exploring the impact of missing data patterns and
    level of missingness on those Fst calculations…

In the absence of missing data, because overall Fst values are similar,
in the end, it’s worth looking at:

- **speed**: if this is an issue for you (e.g. if running multiple
  simulations, assignment analysis, etc).
- **features**: lots of differences between packages/software.

*Please let me know if I did something wrong or forgot to mention an
interesting feature of your favorite package.*
